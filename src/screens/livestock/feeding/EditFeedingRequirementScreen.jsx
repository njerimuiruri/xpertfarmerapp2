...
import { Button, Select, Input, Box, HStack, VStack, FormControl, Radio, TextArea } from 'native-base';
...
const ANIMAL_TYPES = ['Dairy Cattle', 'Beef Cattle', 'Poultry', 'Swine', 'Sheep', 'Goats'];
const FREQUENCY_OPTIONS = ['Once Daily', 'Twice Daily', 'Three Times Daily', 'Weekly', 'Free Choice'];
const UNIT_OPTIONS = [
  { label: 'kg', value: 'kg' },
  { label: 'g', value: 'g' },
...
const FEED_SOURCES = ['Personally Grown', 'Grown and Purchased', 'Purely Purchased'];
...
const getLifecycleOptions = (type) => {
  const options = {
    ...
    'Sheep and Goats': ['Lambs and Kids', 'Growing', 'Production', 'Maintenance'],
  };
...
export default function EditFeedingRequirementScreen({ navigation, route }) {
  ...
  const initialProgramType = requirement.programDetails?.programType || 'Single Animal';
  ...
  const [additionalNotes, setAdditionalNotes] = useState(requirement.additionalNotes || '');
  ...
  const [animalId, setAnimalId] = useState(requirement.programDetails?.animalId || '');
  const [groupId, setGroupId] = useState(requirement.programDetails?.groupId || '');
  const [lifecycleStages, setLifecycleStages] = useState(requirement.programDetails?.lifecycleStages || []);
  const [groupLifecycleStages, setGroupLifecycleStages] = useState(requirement.programDetails?.groupLifecycleStages || []);
  const [feedType, setFeedType] = useState(requirement.programDetails?.feedType || 'Basal Feeds');
  
  // Date picker states
  const [showBasalDatePicker, setShowBasalDatePicker] = useState(false);
  const [showConcentrateDatePicker, setShowConcentrateDatePicker] = useState(false);
  const [showSupplementDatePicker, setShowSupplementDatePicker] = useState(false);

  // Feed details using a reducer pattern
  const createInitialFeedState = (detailsKey) => ({
    ...
    date: new Date(requirement.programDetails?.[detailsKey]?.dateAcquired || new Date()),
    cost: requirement.programDetails?.[detailsKey]?.cost || '',
    supplier: requirement.programDetails?.[detailsKey]?.supplier || '',
  });

  const [basalFeed, setBasalFeed] = useState(createInitialFeedState('basalDetails'));
  const [concentrateFeed, setConcentrateFeed] = useState(createInitialFeedState('concentrateDetails'));
  const [supplementFeed, setSupplementFeed] = useState(createInitialFeedState('supplementDetails'));
  ...
  // Feed form component
  const FeedForm = ({ feed, updateFn, title, showDatePicker, setShowDatePicker, onDateChange }) => (
  ...
  const handleSubmit = () => {
    ...
    // Prepare updated requirement object
    ...
    // Navigate back with updated requirement
    setTimeout(() => {
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 16 }}>
        ...
        <Box bg="white" p={4} borderRadius={8} shadow={1} mb={4}>
          ...
          <FormControl mb={4}>
            ...
            <Radio.Group
              ...
              <VStack space={2}>
                <Radio value="Single Animal" my={1}>Single Animal Feeding Program</Radio>
                <Radio value="Group" my={1}>Group Feeding Program</Radio>
              </VStack>
            </Radio.Group>
          </FormControl>
        </Box>
        ...
        <Box bg="white" p={4} borderRadius={8} shadow={1} mb={4}>
          <Text style={styles.sectionTitle}>
            {programType === 'Single Animal' ? 'Animal Information' : 'Group Information'}
          </Text>
          ...
          <FormControl mb={4}>
            ...
            <Input
              backgroundColor={COLORS.lightGreen}
              placeholder={`Enter ${programType === 'Single Animal' ? 'Animal' : 'Group'} ID`}
              value={programType === 'Single Animal' ? animalId : groupId}
              onChangeText={programType === 'Single Animal' ? setAnimalId : setGroupId}
            />
          </FormControl>
          ...
          <FormControl mb={4}>
            ...
            <Select
              ...
              minWidth="100%"
              backgroundColor={COLORS.lightGreen}
              placeholder={`Select ${programType === 'Single Animal' ? 'Animal' : 'Group'} Type`}
          ...
          {animalType && (
            <FormControl mb={4}>
              ...
              {renderSelectionButtons(
                ...
                programType === 'Single Animal' ? lifecycleStages : groupLifecycleStages,
                item => toggleArrayItem(
                  programType === 'Single Animal' ? lifecycleStages : groupLifecycleStages,
                  programType === 'Single Animal' ? setLifecycleStages : setGroupLifecycleStages,
                  item
                )
              )}
            </FormControl>
        ...
        <Box bg="white" p={4} borderRadius={8} shadow={1} mb={4}>
          ...
          <FormControl mb={4}>
            ...
            <Radio.Group name="feedType" value={feedType} onChange={setFeedType}>
              <VStack space={2}>
                <Radio value="Basal Feeds" my={1}>Basal Feeds</Radio>
        ...
        <Box bg="white" p={4} borderRadius={8} shadow={1} mb={4}>
          ...
          <FormControl mb={4}>
            ...
            <DateInput
              ...
              onChange={handleDateChange(setLastFedDate, setShowLastFedDatePicker)}
            />
          </FormControl>
          ...
          <FormControl mb={4}>
            ...
            <DateInput
              ...
              onChange={handleDateChange(setNextFeedingDate, setShowNextFeedingDatePicker)}
            />
          </FormControl>
            ...
            <HStack flexWrap="wrap" space={2}>
              {TIME_OPTIONS.map((time) => (
                <TouchableOpacity
                  ...
                  onPress={() => toggleArrayItem(timeOfDay, setTimeOfDay, time)}
                >
                  <Text style={[
                    styles.timeSlotText,
        ...
        <Box bg="white" p={4} borderRadius={8} shadow={1} mb={4}>
          ...
          <FormControl mb={4}>
            <FormControl.Label>Nutritional Information</FormControl.Label>
            <Input
              backgroundColor={COLORS.lightGreen}
