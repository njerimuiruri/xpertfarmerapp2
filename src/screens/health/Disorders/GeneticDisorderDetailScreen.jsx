...
const GeneticDisorderDetailScreen = ({ navigation, route }) => {
    const { recordId, recordData } = route.params;

    const [disorderRecord, setDisorderRecord] = useState(recordData || null);
    ...
    const loadDisorderRecord = async () => {
        if (recordData) {
            setDisorderRecord(recordData);
            return;
        }
        ...
        try {
            ...
            setDisorderRecord(result.data);
        } catch (error) {
            console.error('Failed to load genetic disorder record:', error);
            Alert.alert('Error', 'Failed to load genetic disorder record details.');
            navigation.goBack();
        }
    };
    ...
    const getAdministratorTypeDisplay = (type) => {
    ...
    const confirmDelete = async () => {
        try {
            setDeleting(true);
            const result = await deleteGeneticDisorderRecord(recordId);
            ...
            Alert.alert(
                'Success',
                'Genetic disorder record deleted successfully',
                [
                    {
                        text: 'OK',
    ...
    if (isLoading) {
        return (
            <SafeAreaView style={styles.container}>
                <SecondaryHeader
                    title="Disorder Details"
                    onBackPress={() => navigation.goBack()}
                    showNotification={true}
                />
                <View style={styles.loadingContainer}>
                    <ActivityIndicator size="large" color={COLORS.success || '#10B981'} />
                    <Text style={styles.loadingText}>Loading disorder details...</Text>
                </View>
            </SafeAreaView>
        );
    }
            ...
            <SafeAreaView style={styles.container}>
                ...
                <View style={styles.errorContainer}>
                    ...
                    <Text style={styles.errorSubtitle}>
                        The genetic disorder record could not be loaded.
                    </Text>
                    <TouchableOpacity
    ...
    const animal = getAnimalInfo(disorderRecord.livestockId);
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView
                ...
                <View style={styles.headerCard}>
                    <LinearGradient
                        ...
                        <View style={styles.headerContent}>
                            <View style={styles.disorderIconContainer}>
                                <FastImage
                                    source={icons.warning || icons.alert}
                                    style={styles.disorderIcon}
                                    tintColor="#FFFFFF"
                                />
                            </View>
                            <View style={styles.headerInfo}>
                                <Text style={styles.headerTitle}>{disorderRecord.nameOfCondition}</Text>
                                <Text style={styles.headerSubtitle}>Genetic Disorder</Text>
                ...
                <View style={styles.card}>
                    <View style={styles.cardHeader}>
                        <FastImage
                            ...
                            tintColor="#DC2626"
                        />
                        <Text style={styles.cardTitle}>Disorder Details</Text>
                    </View>

                    <View style={styles.detailCard}>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Condition Name</Text>
                            <Text style={styles.detailText}>{disorderRecord.nameOfCondition}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Date Recorded</Text>
                            <Text style={styles.detailText}>{formatDate(disorderRecord.dateRecorded)}</Text>
                        </View>

                        {disorderRecord.animalIdOrFlockId && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailKey}>Animal/Flock ID</Text>
                                <Text style={styles.detailText}>{disorderRecord.animalIdOrFlockId}</Text>
                            </View>
                        )}
                    </View>
                </View>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.detailCard}>
                        ...
                        <View style={styles.remedyContainer}>
                            <Text style={styles.remedyText}>{disorderRecord.remedy}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Administered By</Text>
                            <Text style={styles.detailText}>
                                {getAdministratorTypeDisplay(disorderRecord.administeredByType)}
                            </Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Administrator Name</Text>
                            <Text style={styles.detailText}>{disorderRecord.administeredByName}</Text>
                        </View>

                        {disorderRecord.practiceId && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailKey}>Practice ID</Text>
                                <Text style={styles.detailText}>{disorderRecord.practiceId}</Text>
                            </View>
                        )}

                        {disorderRecord.technicianId && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailKey}>Technician ID</Text>
                                <Text style={styles.detailText}>{disorderRecord.technicianId}</Text>
                            </View>
                        )}
                    </View>
                </View>
                ...
                <View style={styles.card}>
                    <View style={styles.cardHeader}>
                        <FastImage
                            source={icons.info || icons.information}
                            style={styles.cardIcon}
                            tintColor="#6B7280"
                        />
                    ...
                    <View style={styles.detailCard}>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Farm ID</Text>
                            <Text style={styles.detailText}>{disorderRecord.farmId}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Livestock ID</Text>
                            <Text style={styles.detailText}>{disorderRecord.livestockId}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Record ID</Text>
                            <Text style={styles.detailText}>{disorderRecord.id || recordId}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Created</Text>
                            <Text style={styles.detailText}>
                                {disorderRecord.createdAt ? formatDate(disorderRecord.createdAt) : 'N/A'}
                            </Text>
                        </View>

                        {disorderRecord.updatedAt && disorderRecord.updatedAt !== disorderRecord.createdAt && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailKey}>Last Updated</Text>
                                <Text style={styles.detailText}>{formatDate(disorderRecord.updatedAt)}</Text>
                            </View>
                        )}
                    </View>
                </View>
...
export default GeneticDisorderDetailScreen;