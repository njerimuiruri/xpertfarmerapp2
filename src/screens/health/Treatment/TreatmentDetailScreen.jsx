...
const TreatmentDetailScreen = ({ navigation, route }) => {
    ...
    const [treatmentRecord, setTreatmentRecord] = useState(recordData || null);
    ...
    const loadTreatmentRecord = async () => {
        if (recordData) {
            setTreatmentRecord(recordData);
            return;
        }
        ...
        try {
            ...
            setTreatmentRecord(result.data);
        } catch (error) {
            console.error('Failed to load treatment record:', error);
            Alert.alert('Error', 'Failed to load treatment record details.');
            navigation.goBack();
        }
    };
    ...
    const handleEdit = () => {
        navigation.navigate('TreatmentEditScreen', {
            recordId: recordId,
            recordData: treatmentRecord
        });
    };
    ...
    const confirmDelete = async () => {
        try {
            setDeleting(true);
            const result = await deleteTreatment(recordId);
            ...
            <SafeAreaView style={styles.container}>
                ...
                <View style={styles.errorContainer}>
                    ...
                    <Text style={styles.errorSubtitle}>
                        The treatment record could not be loaded.
                    </Text>
                    <TouchableOpacity
    ...
    const animal = getAnimalInfo(treatmentRecord.livestockId);
    const totalCost = (parseFloat(treatmentRecord.costOfDrugs) || 0) + (parseFloat(treatmentRecord.costOfService) || 0);
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView
                    ...
                    <LinearGradient
                        ...
                        <View style={styles.headerContent}>
                            ...
                            <View style={styles.headerInfo}>
                                <Text style={styles.headerTitle}>{treatmentRecord.diagnosis}</Text>
                                <Text style={styles.headerSubtitle}>{treatmentRecord.treatmentType}</Text>
                ...
                <View style={styles.card}>
                    <View style={styles.cardHeader}>
                        <FastImage
                            source={icons.alert || icons.warning}
                            style={styles.cardIcon}
                            tintColor="#EF4444"
                        />
                        <Text style={styles.cardTitle}>Health Event</Text>
                    </View>

                    <View style={styles.detailCard}>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Symptoms</Text>
                            <Text style={styles.detailText}>{treatmentRecord.healthEventSymptoms}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Diagnosis</Text>
                            <Text style={styles.detailText}>{treatmentRecord.diagnosis}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Event Date</Text>
                            <Text style={styles.detailText}>{formatDate(treatmentRecord.healthEventDate)}</Text>
                        </View>

                        {treatmentRecord.animalIdOrFlockId && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailKey}>Animal/Flock ID</Text>
                                <Text style={styles.detailText}>{treatmentRecord.animalIdOrFlockId}</Text>
                            </View>
                        )}
                    </View>
                </View>
                ...
                <View style={styles.card}>
                    <View style={styles.cardHeader}>
                        <FastImage
                            ...
                            tintColor={COLORS.success || '#10B981'}
                        />
                        <Text style={styles.cardTitle}>Treatment Details</Text>
                    </View>

                    <View style={styles.detailCard}>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Treatment Type</Text>
                            <Text style={styles.detailText}>{treatmentRecord.treatmentType}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Treatment Description</Text>
                            <Text style={styles.detailText}>{treatmentRecord.treatmentDescription}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Drug Administered</Text>
                            <Text style={styles.detailText}>{treatmentRecord.drugAdministered}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Dosage</Text>
                            <Text style={styles.detailText}>{treatmentRecord.dosageAdministered} ml</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Date Administered</Text>
                            <Text style={styles.detailText}>{formatDate(treatmentRecord.dateAdministered)}</Text>
                        </View>

                        {treatmentRecord.notes && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailKey}>Notes</Text>
                                <Text style={styles.detailText}>{treatmentRecord.notes}</Text>
                            </View>
                        )}
                    </View>
                </View>
                ...
                <View style={styles.card}>
                    <View style={styles.cardHeader}>
                        <FastImage
                            source={icons.account || icons.user}
                            style={styles.cardIcon}
                            tintColor="#3B82F6"
                        />
                        <Text style={styles.cardTitle}>Medical Officer</Text>
                    </View>

                    <View style={styles.detailCard}>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Officer Name</Text>
                            <Text style={styles.detailText}>{treatmentRecord.medicalOfficerName}</Text>
                        </View>

                        {treatmentRecord.licenseId && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailKey}>License ID</Text>
                                <Text style={styles.detailText}>{treatmentRecord.licenseId}</Text>
                            </View>
                        )}

                        {treatmentRecord.farmerWitnessName && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailKey}>Farmer Witness</Text>
                                <Text style={styles.detailText}>{treatmentRecord.farmerWitnessName}</Text>
                            </View>
                        )}
                    </View>
                </View>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.detailCard}>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Cost of Drugs</Text>
                            <Text style={styles.detailText}>{formatCurrency(treatmentRecord.costOfDrugs)}</Text>
                        </View>

                        <View style={styles.detailRow}>
                            <Text style={styles.detailKey}>Cost of Service</Text>
                            <Text style={styles.detailText}>{formatCurrency(treatmentRecord.costOfService)}</Text>
                        </View>
