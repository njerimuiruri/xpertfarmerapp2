...
import { createBooster, validateBoosterData } from '../../../services/healthservice';
import { getLivestockForActiveFarm } from '../../../services/livestock';

const { width } = Dimensions.get('window');

const AddBoostersRecords = ({ navigation, route }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.boostersOrAdditives.trim()) {
      newErrors.boostersOrAdditives = 'Please specify the booster/additive name';
    }

    if (!formData.purpose.trim()) {
      newErrors.purpose = 'Please enter the purpose of the booster';
    }
    ...
    if (formData.costOfBooster && isNaN(parseFloat(formData.costOfBooster))) {
      newErrors.costOfBooster = 'Please enter a valid cost';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Creating booster with payload:', payload);

      const result = await createBooster(selectedLivestock.id, payload);
      ...
      Alert.alert(
        'Success',
        'Booster record has been added successfully!',
        [
          {
            text: 'OK',
            onPress: () => {
          ...
          {renderFormGroup('Booster Information', (
            <>
              ...
              {renderInput('Purpose', 'purpose', {
                required: true,
                placeholder: 'e.g., Growth promotion, Nutrient deficiency, etc.',
                multiline: true,
                numberOfLines: 2
              })}
            </>
          ...
          {renderFormGroup('Dosage Details', (
            <>
              ...
              {renderInput('Quantity Unit', 'quantityUnit', {
                required: true,
                placeholder: 'e.g., ml, tablets, grams, etc.'
              })}
            </>
          ))}
          ...
          <TouchableOpacity
            ...
            <LinearGradient
              ...
              <Text style={styles.submitText}>
                {isLoading ? 'Adding Booster Record...' : 'Add Booster Record'}
              </Text>
            </LinearGradient>
          </TouchableOpacity>
...
export default AddBoostersRecords;